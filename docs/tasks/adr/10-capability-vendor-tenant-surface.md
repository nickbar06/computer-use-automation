...
| Tenant overlay | Label / emoji / copy | Schema + mock skins; runtime in Task 13 |
...
Task 02 ships tenant skins as fixtures. Task 03 ships overlay JSON (including an emoji name). Task 13 is still the only *runtime* overlay proof and stays optional. REPORT heading 4 must answer both PDF questions even if Task 13 is skipped.
